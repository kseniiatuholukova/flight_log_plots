...
@typechecked
def bin_data(data: pd.Series, n_bins: int, bin_size_threshold: int = 100) -> pd.Series:
    ...
    if bin_size >= bin_size_threshold:
        raise ValueError(
            f"You are creating {n_bins} bins with {bin_size_threshold} datapoints in "
